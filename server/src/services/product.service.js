...
async function getAllProducts(filters = {}) {
  ...
  let query = supabaseAdmin
    ...
    .select(`
      *,
      brand:brands(id, slug, name, logo),
  ...
  if (search) {
    ...
    query = query.or(`name.ilike.%${search}%,specs_short.ilike.%${search}%`);
  }
 ...
 * @returns {object[]}
 */
async function getRelatedProducts(categoryId, excludeSlug, limit = 3) {
  const { data, error } = await supabaseAdmin
    .from('products')
    .select(`
      id, slug, name, specs_short, price, grade, grade_class, icon_type,
      category:categories(id, slug, name, icon),
